...
package Root_Access_Routines;
use Exporter;

$Root_Access_Routines::VERSION = "1.00";

@Root_Access_Routines::ISA = qw( Exporter );
@Root_Access_Routines::EXPORT = qw( get_root_by_su get_root_by_sudo );
...
sub get_root_by_su {
 ...
 #
  if ( ! @root_credentials ) {
    print "$hostname:\tget_root_by_su: no root passwords available\n" if $Rover::debug ;
    return (0);
  }
...
sub get_root_by_sudo {
  ...
  $exp_obj->expect($Root_Access_Routines::command_timeout, '-re', $Rover::user_prompt);

  $exp_obj->send("sudo $Root_Access_Routines::sudo_shell\n");
  select(undef, undef, undef, 0.25);
